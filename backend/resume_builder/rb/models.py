...
class Education(models.Model):
    YEAR_CHOICES = (
        (i, i) for i in range(1980, datetime.datetime.now().year+10)
    )
    _YEAR_CHOICES = (
        (i, i) for i in range(1980, datetime.datetime.now().year + 10)
    )
    ...
    e_yr = models.IntegerField(choices=_YEAR_CHOICES, default=datetime.datetime.now().year+3)
...
class WorkX(models.Model):
    ...
    s_yr = models.IntegerField(choices=YEAR_CHOICES, default=datetime.datetime.now().year)
    e_yr = models.IntegerField(choices=_YEAR_CHOICES, default=datetime.datetime.now().year)
